...
class Task1 : Task
{
    ...
    protected override void Solution()
    {
        ...
        for (int i = 0; i < words.Length; ++i)
        {
            ...
            if (tmpLength >= 50)
            {
                tmpLength -= (words[i].Length + 2);
                cnt = 50 - tmpLength;
                perSpace = cnt / (i - lastIn - 1);
                lastSpace = cnt % (i - lastIn - 1);
        ...
        if (lastIn != words.Length - 1)
        {
            int i = words.Length - 1;
            tmpLength -= 1;
            cnt = 50 - tmpLength;
            perSpace = cnt / (i - lastIn);
            lastSpace = cnt % (i - lastIn);
            for (int j = lastIn; j <= i; ++j)
            {
                answer += words[j];
                answer += " ";
